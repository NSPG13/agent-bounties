...
import "./BoundedAgentWalletV2.sol";

/// @notice Deterministically deploys V2 policy wallets for one canonical bounty factory.
/// @dev The V1 factory and every V1 clone remain unchanged.
contract BoundedAgentWalletV2Factory {
    ...
    event BoundedAgentWalletV2Created(
    ...
    constructor(address bountyFactory_) {
        ...
        implementation = address(new BoundedAgentWalletV2(bountyFactory_));
    }
    ...
    function createWalletAndFund(BoundedAgentWallet.Policy calldata policy, bytes32 userSalt, uint256 initialFunding)
        external
        nonReentrant
        returns (address wallet)
    {
    ...
    function _deploy(
        ...
        if (wallet.code.length > 0) {
            ...
            BoundedAgentWalletV2 existing = BoundedAgentWalletV2(payable(wallet));
        ...
        BoundedAgentWalletV2(payable(wallet)).initialize(owner, policy);
        emit BoundedAgentWalletV2Created(
