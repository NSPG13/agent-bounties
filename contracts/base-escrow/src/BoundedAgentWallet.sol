...
contract BoundedAgentWallet {
    ...
    struct Policy {
        ...
        address deterministicVerifierModule;
        bytes32 signedQuorumVerifierSetHash;
        bytes32 aiJudgeVerifierSetHash;
    }
    ...
    function initialize(address owner_, Policy calldata initialPolicy) external {
        require(msg.sender == deploymentFactory, "not deployment factory");
    ...
    function _configurePolicy(Policy memory nextPolicy) private {
        ...
        if ((nextPolicy.allowedVerificationModes & MODE_DETERMINISTIC) != 0) {
            require(nextPolicy.deterministicVerifierModule.code.length > 0, "deterministic verifier unavailable");
        } else {
            require(nextPolicy.deterministicVerifierModule == address(0), "unused deterministic verifier");
        }
        if ((nextPolicy.allowedVerificationModes & MODE_SIGNED_QUORUM) != 0) {
            require(nextPolicy.signedQuorumVerifierSetHash != bytes32(0), "signed verifier set zero");
        } else {
            require(nextPolicy.signedQuorumVerifierSetHash == bytes32(0), "unused signed verifier set");
        }
        if ((nextPolicy.allowedVerificationModes & MODE_AI_JUDGE_QUORUM) != 0) {
            require(nextPolicy.aiJudgeVerifierSetHash != bytes32(0), "AI verifier set zero");
        } else {
            require(nextPolicy.aiJudgeVerifierSetHash == bytes32(0), "unused AI verifier set");
        }
        ...
        if ((nextPolicy.allowedActions & ALL_ACTIONS) != 0) {
            require(nextPolicy.maxBountyTarget > 0, "bounty target cap zero");
        }
        policy = nextPolicy;
        policyVersion += 1;
        revoked = false;
        ...
        emit PolicyConfigured(
            ...
            nextPolicy.maxBountyTarget,
            nextPolicy.deterministicVerifierModule,
            nextPolicy.signedQuorumVerifierSetHash,
            nextPolicy.aiJudgeVerifierSetHash
        );
    }
    ...
    function _createBounty(
        ...
        bytes32 verifierSetHash = verifiers.length == 0 ? bytes32(0) : keccak256(abi.encode(verifiers));
        _requireVerificationPolicy(params.verificationMode, params.verifierModule, verifierSetHash);
        ...
        require(target <= policy.maxBountyTarget, "bounty target cap exceeded");
    ...
    function _requireBountyPolicy(AgentBounty bounty) private view {
        require(bounty.targetAmount() <= policy.maxBountyTarget, "bounty target cap exceeded");
        _requireVerificationPolicy(bounty.verificationMode(), bounty.verifierModule(), bounty.verifierSetHash());
    }

    function _requireVerificationPolicy(
        AgentBounty.VerificationMode mode,
        ...
        if (mode == AgentBounty.VerificationMode.DeterministicModule) {
            require(verifierModule == policy.deterministicVerifierModule, "deterministic verifier not allowed");
            return;
        }
        if (mode == AgentBounty.VerificationMode.SignedQuorum) {
            require(verifierSetHash == policy.signedQuorumVerifierSetHash, "signed verifier set not allowed");
            return;
        }
        require(verifierSetHash == policy.aiJudgeVerifierSetHash, "AI verifier set not allowed");
    }
