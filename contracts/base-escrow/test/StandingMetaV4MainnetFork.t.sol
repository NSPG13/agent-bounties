...
import "../src/CanonicalIndependentChildVerifierV4.sol";
import "../src/OnchainTermsRegistryV4.sol";
...
contract StandingMetaV4MainnetForkTest {
    ...
    function testOfficialBaseVrfSupportsExactV4GraphAndTwoConsumers() public {
        ...
        require(address(bundle.childFactory()) == BASE_MAINNET_FACTORY, "bundle child factory drift");
        require(address(bundle.stakePool()) == address(pool), "bundle stake pool drift");
        require(address(bundle.verifierSortition()) == address(verifierSortition), "bundle verifier draw drift");
        require(address(bundle.solverSortition()) == address(solverSortition), "bundle solver draw drift");
        require(address(bundle.appealableVerifier()) == address(appeal), "bundle appeal drift");
        require(address(bundle.standingMetaChildFactory()) == address(childFactory), "bundle child module drift");
        require(address(bundle.parentFactory()) == address(parentFactory), "bundle parent factory drift");
        OnchainTermsRegistryV4 termsRegistry = parentFactory.termsRegistry();
        CanonicalIndependentChildVerifierV4 verifierModule = parentFactory.verifierModule();
        require(termsRegistry.publisherAuthority() == address(parentFactory), "terms authority drift");
        require(address(verifierModule.parentFactory()) == address(parentFactory), "verifier parent drift");
        require(verifierModule.canonicalChildFactory() == address(childFactory), "verifier child factory drift");
        require(verifierModule.settlementToken() == BASE_MAINNET_USDC, "verifier token drift");
        require(address(verifierModule.termsRegistry()) == address(termsRegistry), "verifier terms drift");
        require(address(verifierModule.appealableVerifier()) == address(appeal), "verifier appeal drift");
        require(verifierModule.MINIMUM_CHILD_TARGET() == 1_000_000, "child target drift");
        require(verifierModule.MINIMUM_PARENT_MARGIN() == 1_000_000, "parent margin drift");
        require(verifierModule.CHILD_SOLVER_REWARD() == 990_000, "child reward drift");
        require(verifierModule.CHILD_VERIFIER_REWARD() == 10_000, "child verifier reward drift");
