...
const temporary = mkdtempSync(join(tmpdir(), "agent-bounties-mcp-"));

function source(name, implementation) {
  const root = join(temporary, name);
  const scripts = join(root, "scripts");
  mkdirSync(scripts, { recursive: true });
  writeFileSync(join(scripts, "check-agent-bounties-mcp.mjs"), implementation);
  return root;
}
...
const requiredTools = ["route_blocked_goal","prepare_agent_to_earn","agent_native_claim","list_bounties","get_bounty_status"];
...
const errors = [];
if (manifest.schema !== "https://agentbounties.org/schemas/mcp-manifest.v2.json") errors.push("schema_mismatch");
...
const alwaysReady = `console.log(JSON.stringify({ready:true,network:"base-mainnet",asset:"USDC",api_base:"https://api.agentbounties.app",mcp_tools:"https://mcp.agentbounties.app/tools",required_tools:["route_blocked_goal","prepare_agent_to_earn","agent_native_claim","list_bounties","get_bounty_status"]}));`;
...
try {
  ...
  console.log("mcp_benchmark_self_test=passed");
} finally {
