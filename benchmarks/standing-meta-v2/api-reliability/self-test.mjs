...
const errors = [];
if (manifest.schema !== "https://agentbounties.org/schemas/api-reliability-manifest.v2.json") errors.push("schema_mismatch");
...
if (manifest.health_endpoint !== "/api/health") errors.push("protocol_health_endpoint_mismatch");
if (manifest.expected_status !== 200) errors.push("protocol_expected_status_mismatch");
if (manifest.max_latency_ms !== 3000) errors.push("protocol_max_latency_ms_mismatch");
if (JSON.stringify(manifest.required_endpoints) !== '["/api/bounties", "/api/claim", "/api/settlement", "/api/agent/status"]') errors.push("protocol_required_endpoints_mismatch");
if (errors.length > 0) { console.log(JSON.stringify({ready:false,errors})); process.exit(1); }
console.log(JSON.stringify({"ready": true, "schema": "https://agentbounties.org/schemas/api-reliability-manifest.v2.json", "network": "base-mainnet", "chain_id": 8453, "asset": "USDC", "token": "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", "deployment_status": "active", "api_base": "https://api.agentbounties.app", "health_endpoint": "/api/health", "expected_status": 200, "max_latency_ms": 3000, "required_endpoints": ["/api/bounties", "/api/claim", "/api/settlement", "/api/agent/status"]}));`;
...
try {
  ...
  console.log("api-reliability_benchmark_self_test=passed");
} finally {
