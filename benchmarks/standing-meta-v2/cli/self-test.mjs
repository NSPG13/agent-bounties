...
const temporary = mkdtempSync(join(tmpdir(), "agent-bounties-cli-benchmark-"));
...
const requiredCommands = ["claim-next-action", "select-funded-bounty", "verify-settlement-evidence", "prepare-child-bounty"];
...
const errors = [];
if (manifest.schema !== "https://agentbounties.app/schemas/cli-manifest.v1.json") errors.push("schema_mismatch");
...
if (endpoints.cli_home !== "https://agentbounties.app/cli") errors.push("cli_home_mismatch");
...
console.log(JSON.stringify({ready:true,network:"base-mainnet",asset:"USDC",api_base:"https://api.agentbounties.app",cli_home:"https://agentbounties.app/cli",required_commands:requiredCommands}));
`;

const alwaysReady = `
console.log(JSON.stringify({ready:true,network:"base-mainnet",asset:"USDC",api_base:"https://api.agentbounties.app",cli_home:"https://agentbounties.app/cli",required_commands:["claim-next-action","select-funded-bounty","verify-settlement-evidence","prepare-child-bounty"]}));
`;
