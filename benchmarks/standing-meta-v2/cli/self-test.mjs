...
const errors = [];
if (m.schema !== "https://agentbounties.org/schemas/cli-manifest.v2.json") errors.push("schema_mismatch");
if (m.binary !== "agent-bounty-cli") errors.push("binary_mismatch");
const commands = Array.isArray(m.commands) ? m.commands : [];
for (const c of ["list","claim","register","status"]) { if (!commands.includes(c)) errors.push("command_missing:" + c); }
const codes = Array.isArray(m.exit_codes) ? m.exit_codes : [];
if (!codes.includes(2)) errors.push("exit_code_mismatch:2");
if (m.help !== true) errors.push("help_missing");
if (m.node_version !== ">=20") errors.push("node_version_mismatch");
if (errors.length > 0) { console.log(JSON.stringify({ready:false,errors})); process.exit(1); }
console.log(JSON.stringify({ready:true,binary:"agent-bounty-cli",commands:["list","claim","register","status"],exit_codes:[0,1,2],help:true,node_version:">=20"}));
`;
