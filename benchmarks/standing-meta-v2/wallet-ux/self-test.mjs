...
const requiredFeatures = ["gasless_claim","sponsored_creation","bond_refund","wallet_relay"];
...
const features = Array.isArray(manifest.wallet_features) ? manifest.wallet_features : [];
const errors = [];
...
if (manifest.wallet_provider !== "bounded-wallet") errors.push("wallet_provider_mismatch");
if (endpoints.api_base !== "https://api.agentbounties.app") errors.push("api_endpoint_mismatch");
for (const feat of requiredFeatures) if (!features.includes(feat)) errors.push("required_feature_missing:" + feat);
if (errors.length > 0) { console.log(JSON.stringify({ready:false,errors})); process.exit(1); }
console.log(JSON.stringify({ready:true,network:"base-mainnet",asset:"USDC",api_base:"https://api.agentbounties.app",wallet_provider:"bounded-wallet",required_features:requiredFeatures}));`;

const alwaysReady = `console.log(JSON.stringify({ready:true,network:"base-mainnet",asset:"USDC",api_base:"https://api.agentbounties.app",wallet_provider:"bounded-wallet",required_features:["gasless_claim","sponsored_creation","bond_refund","wallet_relay"]}));`;
