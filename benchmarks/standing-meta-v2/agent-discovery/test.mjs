...
const ready = {"ready": true, "feed_url": "https://api.agentbounties.app/v1/base/autonomous-bounties/feed?network=base-mainnet", "network": "base-mainnet", "refresh_interval_seconds": 300, "discovery_fields": ["bounty_id", "platform", "title", "status"], "indexed": ["claimable", "claimed", "settled"]};

const invalidErrors = ["schema_mismatch", "feed_url_mismatch", "network_mismatch", "discovery_field_missing:bounty_id", "discovery_field_missing:platform", "discovery_field_missing:title", "discovery_field_missing:status", "refresh_interval_mismatch", "indexed_missing:claimable"];
...
const cases = [
  ...
  { name: "wrong feed scheme", args: [join(benchmarkRoot, "fixtures", "wrong-protocol.json")], status: 1, output: { ready: false, errors: invalidErrors } },
