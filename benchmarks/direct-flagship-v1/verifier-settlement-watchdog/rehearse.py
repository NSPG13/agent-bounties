  ...
  "jobs": {
    "run-no-secrets": {
      ...
      "steps": [
        ...
        {"name": "Verify reviewed worker build inputs", "run": "python scripts/regression_verifier_source_guard.py --scope worker-build --expected-sha256 a284b56b70522433b5ec3a3711e1f110aeb9d321bae3792fc07b56d512beda24"},
        {"name": "Build isolated regression worker", "run": "cargo build --release -p worker"},
        {"name": "Revalidate reviewed sources after build", "run": "python scripts/regression_verifier_source_guard.py --scope worker-build --expected-sha256 a284b56b70522433b5ec3a3711e1f110aeb9d321bae3792fc07b56d512beda24"},
        {"name": "Revalidate reviewed signing runtime", "run": "python scripts/regression_verifier_source_guard.py --scope signing-runtime --expected-sha256 6b0c5073097e9612f94e35089285729d0c0f5db580f75b351d19a97f8bffd2f1"},
        {"name": "Run canonical jobs without signing secrets", "run": "python scripts/regression_verifier_pipeline.py run --api-base $API_BASE_URL --network base-mainnet --verifier $VERIFIER_ONE --verifier $VERIFIER_TWO --worker target/release/worker --staging $RUNNER_TEMP/regression-staging --output target/regression-candidates --max-jobs 5 --max-scan 100"},
        {"uses": "actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a", "if": "always()", "with": {"name": "regression-candidates-${{ github.run_id }}", "path": "target/regression-candidates", "if-no-files-found": "error", "retention-days": 7}}
      ]
    }
  }
}'''
...
with tempfile.TemporaryDirectory(prefix="watchdog-line-ending-mutation-") as temporary:
    ...
    if b"\r\n" in original and b"\r" not in without_crlf and b"\n" not in without_crlf:
        changed = original.replace(b"\r\n", b"\n")
    elif b"\r" not in original and b"\n" in original:
        changed = original.replace(b"\n", b"\r\n")
    else:
        raise SystemExit("line-ending rehearsal requires one consistent reviewed convention")
    migration.write_bytes(changed)
    result = check(mutated)
    if result.returncode == 0:
