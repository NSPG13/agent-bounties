#!/usr/bin/env python3
"""Verify production API, MCP and discovery surfaces expose one Beta2 release."""
...
import wait_open_competition_v2_beta2_runtime as runtime_wait
...
def contains_beta2(value: Any) -> bool:
    if isinstance(value, str):
        return "open-competition-v2-beta2" in value or "open_competition_v2" in value
    if isinstance(value, dict):
        return any(contains_beta2(key) or contains_beta2(item) for key, item in value.items())
    if isinstance(value, list):
        return any(contains_beta2(item) for item in value)
    return False
...
def verify(api: str, mcp: str, expected: dict[str, Any]) -> dict[str, Any]:
    api = api.rstrip("/")
    mcp = mcp.rstrip("/")
    api_release = request_json("GET", f"{api}/v1/base/open-competition-v2-beta2/release?network=base-mainnet")
    ...
    if not runtime_wait.exact_runtime(api_release, expected, True, False):
        raise InterfaceError("API release or indexer agreement differs from expected Beta2 runtime")
    if not runtime_wait.exact_runtime(mcp_release, expected, True, False):
        raise InterfaceError("MCP release proxy differs from expected Beta2 runtime")
    ...
    if not contains_beta2(api_discovery) or not contains_beta2(mcp_discovery):
        raise InterfaceError("API or MCP discovery manifest omits Beta2")
    canonical = json.dumps(expected, sort_keys=True, separators=(",", ":")).encode()
    return {
        "schema_version": "agent-bounties/open-competition-v2-beta2-interface-agreement-v1",
        "passed": True,
        ...
        "api_discovery_beta2": True,
        "mcp_discovery_beta2": True,
