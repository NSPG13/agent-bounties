...
if [[ -r /proc/meminfo ]]; then
  ...
  if (( memory_kib < minimum_memory_kib )); then
    echo "circuit build requires at least 250 GiB reported physical memory; swap does not qualify" >&2
    exit 2
  fi
fi
...
available_kib="$(df -Pk "$source_root" 2>/dev/null | awk 'NR == 2 { print $4 }')"
minimum_available_kib=$((60 * 1024 * 1024))
if [[ -n "$available_kib" ]] && (( available_kib < minimum_available_kib )); then
