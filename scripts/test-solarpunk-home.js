...
test("bounty assistant handoffs carry one bounded initialization message", () => {
  ...
  const claudeCode = home.bountyAssistantLinks("claude-code");
  ...
  // "Claude" opens the Claude Desktop chat app; Claude Code has its own entry and scheme.
  assert.equal(claude.desktopUrl, `claude://claude.ai/new?q=${encodeURIComponent(prompt)}`);
  assert.equal(new URL(claude.desktopUrl).protocol, "claude:");
  assert.equal(new URL(claude.desktopUrl).searchParams.get("q"), prompt);
  assert.equal(claude.desktopLabel, undefined);
  assert.equal(claude.terminalCommand, undefined);
  assert.equal(new URL(claude.webUrl).origin, "https://claude.ai");
  assert.equal(new URL(claude.webUrl).searchParams.get("q"), prompt);
  assert.equal(claudeCode.desktopUrl, `claude-cli://open?q=${encodeURIComponent(prompt)}`);
  assert.equal(new URL(claudeCode.desktopUrl).protocol, "claude-cli:");
  assert.equal(claudeCode.desktopLabel, "Claude Code");
  assert.equal(claudeCode.webUrl, claude.webUrl);
...
test("a prompt beyond the documented Claude Code limit takes the web handoff", () => {
  ...
  const links = home.bountyAssistantLinks("claude-code", long);
  assert.equal(links.desktopUrl, null);
  assert.equal(new URL(links.webUrl).searchParams.get("q"), long);
  // The Claude Desktop scheme has no documented limit and keeps the whole message.
  assert.equal(new URL(home.bountyAssistantLinks("claude", long).desktopUrl).searchParams.get("q"), long);
  ...
  assert.equal(home.bountyAssistantLinks("claude-code", fits).desktopUrl, `claude-cli://open?q=${fits}`);

  // Percent-encoding counts: 2,000 spaces encode to 6,000 characters.
  const encodesOver = " ".repeat(2000);
  assert.equal(home.bountyAssistantLinks("claude-code", encodesOver).desktopUrl, null);
...
test("the desktop status names the recovery for an unclaimed scheme", () => {
  const claude = home.bountyAssistantLinks("claude-code");
  ...
  for (const provider of ["gpt", "claude", "cursor"]) {
...
test("desktop schemes are only fired where a desktop app can register one", () => {
  ...
  // Every desktop platform may run an app that registers these schemes.
