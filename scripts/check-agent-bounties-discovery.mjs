 ...
 * @param {number} status - Process exit status code.
 * @param {string[]} errors - Array of machine-readable error codes.
 * @returns {never}
 */
...
/**
 * Validate Agent Bounties discovery manifest file against canonical protocol requirements.
 * @returns {void}
 */
function validateManifest() {
  if (process.argv.length !== 3) {
    exitWithError(2, ["manifest_path_required"]);
  }

  const manifestPath = process.argv[2];
  let fileContent = "";
  try {
    fileContent = readFileSync(manifestPath, "utf8");
  } catch {
    exitWithError(2, ["manifest_unreadable"]);
  }
  ...
  try {
    ...
    exitWithError(2, ["manifest_invalid_json"]);
  }

  if (manifest === null || Array.isArray(manifest) || typeof manifest !== "object") {
    exitWithError(2, ["manifest_root_object_required"]);
  }
