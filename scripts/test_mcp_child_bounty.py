...
class McpChildBountyTests(unittest.TestCase):
    ...
    def test_mcp_discovery_benchmark_self_test_passes(self) -> None:
        """
        Execute the pinned mcp-discovery self-test harness.
        """
        self_test_script = ROOT / "benchmarks" / "standing-meta-v2" / "mcp-discovery" / "self-test.mjs"
        result = subprocess.run(
            ["node", str(self_test_script)],
            capture_output=True,
            text=True,
            check=False,
        )
        self.assertEqual(
            result.returncode,
            0,
            f"mcp-discovery self-test failed:\nstdout: {result.stdout}\nstderr: {result.stderr}",
        )
        self.assertIn("mcp_discovery_benchmark_self_test=passed", result.stdout)
