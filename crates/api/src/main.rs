...
mod tests {
    ...
    /// Issue #684: deterministic fail-closed test for stale domain origins.
    ///
    /// Acceptance criteria:
    /// - A retired legacy origin is rejected OR absent from generated output.
    /// - Canonical website, API, and MCP origins remain accepted where their
    ///   surface requires them.
    /// - The test covers both origin validation and at least one generated
    ///   link or redirect.
    /// - No test relies on DNS, external HTTP, credentials, or a live wallet.
    /// - The test exits 0 on the committed repository state.
    ///
    /// Design: the canonical surface set is {agentbounties.app,
    /// api.agentbounties.app, mcp.agentbounties.app}. Any origin that is not
    /// a first-party canonical host (or an explicitly allow-listed alias in
    /// `site_analytics_origin_allowed`) must be rejected from analytics. Any
    /// redirect generated by `marketing_domain_destination` must land on a
    /// canonical HTTPS base — never on http://, an empty string, or a
    /// self-loop. The test pins both the origin gate and the redirect output
    /// so a future refactor cannot silently leak a stale origin into either.
    #[test]
    fn stale_domain_origins_fail_closed_and_canonical_origins_remain_accepted() {
        // --- Origin validation: site analytics surface ---
        // Canonical website origin must be accepted.
        let mut headers = HeaderMap::new();
        headers.insert(
            header::ORIGIN,
            HeaderValue::from_static("https://agentbounties.app"),
        ...
        assert!(
            ...
            "canonical website origin must be accepted by site analytics"
        );

        // Arbitrarily hostile / unknown origins must be rejected. This is the
        // fail-closed guarantee: anything that is not explicitly allowed is
        // denied, with no fuzzy matching that could leak a lookalike through.
        for hostile_origin in [
            // Look-alike subdomain attacks.
            "https://agentbounties.app.evil.example",
            "https://evil.agentbounties.app",
            "https://agentbounties.app.evil",
            // A retired origin that is NOT in the allow-list (bountyboard.global
            // IS allow-listed for backward compatibility, so we test a sibling
            // retired host that is not).
            "https://bountyboard.global.evil.example",
            "https://agentbounties.evil",
            // Non-HTTPS variants must be rejected even if the host is canonical.
            "http://agentbounties.app",
            "http://api.agentbounties.app",
            "http://mcp.agentbounties.app",
            // Empty / malformed.
            "",
            "not-a-url",
        ] {
            headers.insert(
                header::ORIGIN,
                HeaderValue::from_str(hostile_origin).unwrap_or(
                    HeaderValue::from_static("")
                ),
            );
            assert!(
                !site_analytics_origin_allowed(&headers),
                "hostile/stale origin {:?} must be rejected by site analytics",
                hostile_origin
            );
        }

        // --- Generated links / redirects: marketing alias surface ---
        // Every redirect emitted by marketing_domain_destination must land on
        // a canonical HTTPS base (agentbounties.app or api.agentbounties.app).
        // No redirect may target a non-canonical alias, http://, or empty string.
        let canonical_redirect_bases = [
            "https://agentbounties.app",
        ...
        for alias_host in [
            ...
            let destination = marketing_domain_destination(
                alias_host,
                &"/earn.html?from=legacy".parse().unwrap(),
            );
            let dest = destination.unwrap_or_else(|| panic!(
                "alias host {alias_host} must produce a redirect destination"
            ));
            assert!(
                dest.starts_with("https://"),
                "redirect for {alias_host} must be HTTPS, got: {dest}"
            );
            assert!(
                canonical_redirect_bases
                    .iter()
                    .any(|base| dest.starts_with(base)),
                "redirect for {alias_host} must land on a canonical base, got: {dest}"
            );
            // The alias itself must not appear as the destination host.
            let dest_host = dest
                .strip_prefix("https://")
                .unwrap_or(&dest)
                .split('/')
                ...
                .unwrap_or("");
            assert_ne!(
                dest_host, alias_host,
                "redirect for {alias_host} must not self-loop"
            );
        }

        // --- Canonical hosts must NOT be treated as marketing aliases ---
        // api.agentbounties.app and mcp.agentbounties.app are canonical API
        // and MCP surfaces, not marketing aliases. They must not redirect.
        for canonical_host in [
            ...
            assert_eq!(
                ...
                None,
                "canonical host {canonical_host} must not be treated as a marketing alias"
            );
        }

        // --- Generated link evidence: canonical origins appear in committed
        // static URL constants, never as alias origins. This guards against
        // drift where a refactor accidentally points a canonical surface at
        // a retired host. ---
        for url in [
            ...
            assert!(url.starts_with("https://agentbounties.app/"));
            assert!(!url.contains("bountyboard.global"));
            assert!(!url.contains("agentbounties.io"));
        }
    }
