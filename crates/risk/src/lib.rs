...
pub fn validate_direct_bounty_evidence(
    ...
    if evidence.subdirectory != "."
        && !evidence
            .subdirectory
            .split('/')
            .all(is_canonical_path_segment)
    {
        errors.push("subdirectory must be a canonical relative path or .".to_string());
    }
...
fn github_path_parts(url: &str) -> Option<Vec<&str>> {
    let rest = url.strip_prefix("https://github.com/")?;
    let parts: Vec<_> = rest.split('/').collect();
    if parts.len() < 2
        || !parts.iter().all(|part| is_canonical_path_segment(part))
        || !github_repository_segments(parts[0], parts[1])
    {
        return None;
    }
    Some(parts)
...
fn github_repository_segments(owner: &str, repo: &str) -> bool {
    ...
    allowed(owner)
        && allowed(repo)
        && is_canonical_path_segment(owner)
        && is_canonical_path_segment(repo)
        && !sanitize_repo_name(repo).is_empty()
}

fn parse_github_repo(url: &str) -> Option<(String, String)> {
    let parts = github_path_parts(url)?;
    (parts.len() == 2).then(|| (parts[0].to_ascii_lowercase(), sanitize_repo_name(parts[1])))
}

fn numeric_github_id(value: &str) -> bool {
    !value.is_empty()
...
fn parse_github_pr_repo(url: &str) -> Option<(String, String)> {
    let parts = github_path_parts(url)?;
    (parts.len() == 4 && parts[2] == "pull" && numeric_github_id(parts[3]))
        .then(|| (parts[0].to_ascii_lowercase(), sanitize_repo_name(parts[1])))
}

/// Exact Actions/Checks routes only; suffixes cannot normalize into another route.
...
fn parse_github_repo_from_check_run_url(url: &str) -> Option<(String, String)> {
    let parts = github_path_parts(url)?;
    let valid = match parts.get(2..)? {
        ["actions", "runs", id] | ["runs", id] => numeric_github_id(id),
        ["actions", "runs", id, "jobs", job] => numeric_github_id(id) && numeric_github_id(job),
        ["actions", "runs", id, "attempts", attempt] => {
            numeric_github_id(id) && numeric_github_id(attempt)
        }
        _ => false,
    };
    valid.then(|| (parts[0].to_ascii_lowercase(), sanitize_repo_name(parts[1])))
}
...
fn is_canonical_path_segment(segment: &str) -> bool {
    ...
    !segment
        .chars()
        .any(|c| c == '\\' || c.is_whitespace() || c.is_control() || "%?#".contains(c))
}
...
fn parse_immutable_artifact_url(url: &str) -> Option<ParsedArtifactUrl> {
    ...
    if let Some(rest) = url.strip_prefix("https://raw.githubusercontent.com/") {
        let parts: Vec<&str> = rest.split('/').collect();
        if parts.len() >= 4 && github_repository_segments(parts[0], parts[1]) {
            let sha = parts[2];
...
mod tests {
    ...
    #[test]
    fn evidence_urls_reject_normalization_and_route_confusion() {
        for path in [
            "owner/repo/actions/runs/123/../../issues/686",
            "owner/repo/actions/runs/123/jobs/456/../../../issues/686",
            "owner/repo/actions//runs/123",
            "owner/repo/actions/runs/123/unknown",
            "owner/repo/runs/123/..",
            "owner/repo/actions/runs/0",
            "owner/repo/actions/runs/123\\\t",
            "owner/repo/actions/runs/123%2f..",
        ] {
            assert!(
                !is_github_check_run_url(&format!("https://github.com/{path}")),
                "{path}"
            );
        }
        assert!(
            parse_github_pr_repo("https://github.com/owner/repo/pull/123/../../issues/686")
                .is_none()
        );
        assert!(parse_github_pr_repo("https://github.com/owner/repo/pull/0").is_none());
        for repo in [
            "owner//repo",
            "owner/..",
            "owner/.git",
            ...
            assert!(parse_github_repo(&format!("https://github.com/{repo}")).is_none());
            assert!(!is_immutable_artifact_url(&format!("https://raw.githubusercontent.com/{repo}/0123456789abcdef0123456789abcdef01234567/report.json")));
        }
    }
}
